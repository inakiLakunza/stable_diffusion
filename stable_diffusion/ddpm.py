...
class DDPMSampler:
    ...
    def __init__(self, generator: torch.Generator, num_training_steps=1000, beta_start: float=0.00085, beta_end: float=0.0120):
        ...
        self.alpha_cumprod = torch.cumprod(self.alphas, 0) # [alpha_0, alpha_0 * alpha_1, alpha_0 * alpha_1 * alpha_2, ...] Cumulative product
    ...
    def step(self, timestep: int, latents: torch.Tensor, model_output: torch.Tensor):
        ...
        alpha_prod_t = self.alpha_cumprod[timestep]
        alpha_prod_t_prev = self.alpha_cumprod[prev_t] if prev_t >= 0 else self.one # There is a paper claiming that this is not correct
        beta_prod_t = 1 -alpha_prod_t
        beta_prod_t_prev_t = 1 - alpha_prod_t_prev
        ...
        curent_sample_coeff = current_alpha_t ** 0.5 * beta_prod_t_prev_t / beta_prod_t

        # Compute the predicted previous  sample mean
        pred_prev_sample = pred_original_sample_coeff * pred_original_sample + curent_sample_coeff * latents
    ...
    def add_noise(self, original_samples: torch.FloatTensor, timesteps: torch.IntTensor) -> torch.FloatTensor:
        alpha_cumprod = self.alpha_cumprod(device=original_samples.device, dtype=original_samples.dtype)
